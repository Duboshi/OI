...
NOIP 03.1 一维数组
	...
	本章内容较多，有一维数组、二维数组、字符数组、字符串等，本文件仅包含一维数组 
 */
...
void printArray(bool arg [], int length) 
{
	for (i=0; i<length; i++)
    	printf("%d ", arg[i]);
 	printf("\n");
...
int main()
{
	...
	printf("例1：一维数组\n");
		...
		1. 每个数组由多个数据类型相同的元素组成，可以是整数、实数、字符等等；
		2. 建立数组时[]里面是元素的个数，一定是常量，不可以是变量；
		3. 数组中元素按照0, 1, 2...排序，注意是从0而不是1开始，a[0]表示数组a中的
			第1个元素，如果数组a有7个元素，则最后一个元素为a[6]；
		4. 在调用数组中的元素时，[]里面可以是变量；
		...
		*/
		...
		//像上面代码这样依次对数组中每个元素均做一次且仅做一次访问的做法叫做
		//【遍历Traversal】 
	...
	printf("例2：一维数组的应用 - 开关灯\n"); 
		...
		*/
		int k, first=1;
		bool light [1050] = {}; //所有都是false，对应数字0 
		scanf("%d %d", &n, &k);
		//printArray(light, n);
		for(i=1; i<=k; i++)
		{
	...
	/*
	排序与搜索是算法与数据机构领域的重要问题，也是算法竞赛中占有重要地位，
	...
	*/
	...
	printf("例5：一维数组的应用 - 顺序搜索Sequential search\n"); 
	...
	printf("例6：一维数组的应用 - 选择排序Selection sort\n"); 
	...
	printf("例7：一维数组的应用 - 冒泡排序Bubble sort\n"); 
	...
	printf("例8：一维数组的应用 - 插入排序Insertion sort\n");
	...
	printf("例9：一维数组的应用 - 计数排序Counting sort\n"); 
	...
	printf("例10：一维数组的应用 - 二分搜索Binary search\n"); 
