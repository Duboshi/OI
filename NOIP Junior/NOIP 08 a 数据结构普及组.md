...
1968年[Donald E. Knuth（高德纳）](https://en.wikipedia.org/wiki/Donald_Knuth)的*The Art of Computer Programming*（简称TAOCP）第一卷Volume 1: Fundamental Algorithms出版，书中详细讨论了数据的逻辑结构和存储结构，开创了数据结构（他在书中称为信息结构Information structure）的最初体系。从那时起《数据结构》开始成为一门独立的课程，至今仍是计算机专业的核心课程之一。  
...
* 所谓逻辑方面的区别，就是不同数据结构中数据间的逻辑关系不一样。按照这个角度，数据结构可以分为四类：  
	* **集合（逻辑）结构**：如果我们记录学校各个社团的名单，这时候把每位学生看作一个数据，只关心每名同学是否属于某个社团，适合用集合结构；
	* **线性（逻辑）结构**：如果要把班级同学按照身高排序可以使用数组array，数组是一种我们已经接触过的数据结构；多台电脑共用一台打印机时，打印机按照各台电脑提交打印任务的先后顺序处理，这时候使用的数据结构是队列list，有先进先出(first-in-first-out, FIFO)的特性；浏览网页时我们不断点击“后退”按钮，浏览器就会不断逆向显示之前显示过的页面，这时存储历史记录的数据结构是栈stack，有后进先出(last-ink-first-out, LIFO)的特性。数组、队列、栈都是线性数据结构的具体例子；  
	* **树形（逻辑）结构**：计算机目录结构、互联网域名、社会组织等都具有鲜明的层次化特征，使用树结构更为合适。树属于**非线性结构**，但由于在附加某种约束之后，也可以在树中的元素之间确定某种线性次序，因此树结构也被称为**半线性结构**；
...
* 所谓存储方面的区别，就是不同数据结构存储数据的形式不同，主要分成两类：
	...
	* 现在举例子就扯得太远了，讲到线性结构时我们再举例说明两种存储结构的区别。
...
2. **迭代器Iterator**：用于访问容器中元素的工具。迭代器是个变量，指向容器中的某个元素，作用类似于指针，```* 迭代器```就表示迭代器指向的元素。迭代器按照定义方式可以分为以下4种：
	...
	4. **常量反向迭代器**：不能修改其指向的元素，同理要注意与**反向迭代器常量**的区别，这里不赘述
		...
		对于反向迭代器和常量反向迭代器，```++迭代器```都表示指向前一个元素，而```--迭代器```则表示指向后一个元素。下例演示了如何通过反向迭代器遍历一个vector容器：  
	...
	1. **单向迭代器**：只能```++i;```移动（i是迭代器）  
	2. **双向迭代器**：可以```++i;```和```--i;```移动  
	3. **随机访问迭代器**：具备双向迭代器的功能，还支持```i+=c;```、```i-=c;```（c是整数常量）等移动方式。而且两个随机访问迭代器之间可以用>、<、>=、<=等进行比较以及-进行计算，```i1 < i2```的含义是i1指向元素的地址小于i2指向元素的地址，```i1-i2```的返回值是i1指向元素和i2指向元素的序号之差  

	不同容器上的迭代器功能不一样，我们在vector容器迭代器使用的功能在其他容器上未必可用：  
