...
* **容器**：  
	* STL容器vector，我们常用的库函数如下表，具体例子见下文例2.（《新标准》P334，《基础篇》P234）：  
		...
		|front()|访问函数：返回向量首元素的引用，注意不要与迭代器函数```beigin()```混淆|  
		...
		|insert()|修改函数：```v.insert(pos, value)```在向量v中迭代器pos指向的元素【之前】插入一个值为value的元素；```v.insert(pos, n, value)```在向量v中迭代器pos指向的元素【之前】插入n个值为value的元素；```v.insert(pos, first, last)```在向量v中迭代器pos指向的元素【之前】插入向量迭代器区间（或是数组指针区间）[first, last)内的元素。函数返回值为迭代器pos。|  
