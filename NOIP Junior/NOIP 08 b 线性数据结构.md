...
* **特征**：上文已知，顺序表的优点是访问效率高（常数时间复杂度），而插入、删除等效率并不理想（线性时间复杂度）。链表与之相反：逻辑上相邻的元素存储位置并不相邻，它们靠指针彼此联系起来，如图所示：
[这里应该有个图]
	* 历史：人工智能这个领域诞生于1956年达特茅斯学院召开的一次会议，1957年在RAND公司做人工智能研究的Allen Newell、Cliff Shaw和Herbert A. Simon三位学者为了模拟  
* **原理**：  
* **容器**：  
