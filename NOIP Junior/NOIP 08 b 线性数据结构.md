...
* **特征**：  
	...
	* 访问：循链访问的效率低于顺序表的寻秩访问，例如当我们想知道第i个元素的值时，必须从头开始，沿着图中蓝色链箭头逐个访问，直至第i个（或者从尾开始沿着黄色链箭头逐个访问）。对于长度为n的链表，如果访问各个元素的概率相等，那么访问操作的时间复杂度为O(n)  
	* 插入：只要指定了插入位置，则插入一个元素需要改变4个指针的值，即为常数时间复杂度O(1)  
	* 删除：只要指定了插入位置，则插入一个元素需要改变4个指针的值，即为常数时间复杂度O(1)。所以插入和删除是链表的效率优势所在，也是Simon和Newell发明这种数据结构的目的所在。  
* **原理**：  
	* 单链表：《基础篇》P130-133  
